...
import type { AnthropicToolsState } from "../anthropicTools.js";
...
describe("StateFileSearchMiddleware", () => {
  ...
  describe("glob_search", () => {
    ...
    it("should support **/ glob pattern", async () => {
      ...
      expect((result as ToolMessage).content).toContain("/src/utils/helpers.ts");
      expect((result as ToolMessage).content).toContain("/src/index.ts");
      expect((result as ToolMessage).content).not.toContain("/src/components/Button.tsx");
    });
  ...
  describe("grep_search", () => {
    ...
    it("should return matching lines (content mode)", async () => {
      const state: AnthropicToolsState = {
        text_editor_files: {
          "/file1.txt": {
            content: ["line 1 with hello", "line 2 without", "line 3 with hello"],
            created_at: "2024-01-01T00:00:00Z",
            modified_at: "2024-01-01T00:00:00Z",
          },
        },
      ...
      expect((result as ToolMessage).content).toContain("/file1.txt:1:line 1 with hello");
      expect((result as ToolMessage).content).toContain("/file1.txt:3:line 3 with hello");
    ...
    it("should return error for invalid regex", async () => {
      ...
      expect((result as ToolMessage).content).toContain("Invalid regex pattern");
    });
...
describe("FilesystemFileSearchMiddleware", () => {
  ...
  describe("glob_search", () => {
    ...
    it("should support recursive glob patterns", async () => {
      ...
      fs.writeFileSync(path.join(tempDir, "src", "utils", "helper.ts"), "content");
  ...
  describe("grep_search", () => {
    ...
    it("should return matching lines (content mode)", async () => {
      ...
      expect((result as ToolMessage).content).toContain("/test.txt:1:line 1 with hello");
      expect((result as ToolMessage).content).toContain("/test.txt:3:line 3 with hello");
      expect((result as ToolMessage).content).not.toContain("line 2 without");
    });
