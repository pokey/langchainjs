...
import type { AnthropicToolsState, FileData } from "./anthropicTools.js";
...
syntax and filters files by pattern with the include parameter.`,
  ...
  protected handleGlobSearch(request: import("./types.js").ToolCallRequest): ToolMessage {
    const args = (request.toolCall.args || {}) as z.infer<typeof globSearchSchema>;
    ...
    const files = (state[
      this.stateKey as keyof AnthropicToolsState
  ...
  protected handleGrepSearch(request: import("./types.js").ToolCallRequest): ToolMessage {
    const args = (request.toolCall.args || {}) as z.infer<typeof grepSearchSchema>;
    const pattern = args.pattern;
    const basePath = args.path.startsWith("/") ? args.path : `/${args.path}`;
...
export class FilesystemFileSearchMiddleware implements AgentMiddleware {
  ...
  constructor(options: {
    ...
    this.useRipgrep = options.useRipgrep !== undefined ? options.useRipgrep : true;
...
syntax and filters files by pattern with the include parameter.`,
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            if (matchGlobPattern(relativePath, pattern)) {
              const virtualPath = `/${path.relative(this.rootPath, fullPath).replace(/\\/g, "/")}`;
  ...
  protected async handleGrepSearch(request: import("./types.js").ToolCallRequest): Promise<ToolMessage> {
    const args = (request.toolCall.args || {}) as z.infer<typeof grepSearchSchema>;
    const pattern = args.pattern;
    const basePath = args.path;
    const include = args.include;
    ...
    for (const line of stdout.split("\n")) {
      ...
      try {
        ...
        if (data.type === "match") {
          ...
          const virtualPath = `/${path.relative(this.rootPath, filePath).replace(/\\/g, "/")}`;
